...
function BlogPost({
  ...
  const highlightText = (text, term) => { //function to highlight the searched text
    ...
    return text.split(regex).map((part, index) =>   //splits the text by search term matches
      regex.test(part) ? (     //check if the current part matches the search term
        <span key={index} className={styles.highlight}>  //if matched wrap it with span with highlight class
          {part}
        </span>
      ) : (
  ...
  return (
    <article className={`${styles.blogPost} ${isDarkMode ? styles.dark : ""}`}>  {/* renders the blogpost and applies darkmode class if isDarkMode is true */}
      {image && (   //if image is present render the image
